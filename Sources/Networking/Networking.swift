...
public protocol Network {
    ...
    func upload(_ request: URLRequest, from: Data) async throws -> HTTPResponse
}
...
extension URLSession: Network {
    ...
    public func upload(_ request: URLRequest, from data: Data) async throws -> HTTPResponse {
        let (data, response) = try await upload(for: request, from: data)
